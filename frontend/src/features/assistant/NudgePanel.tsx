...
function ContextBar({ theme }: { theme: any }) {
  ...
  return (
    ...
    >
      <p style={{
        ...
        textTransform: 'uppercase', color: '#9b8ea5',
      ...
      {[
        { label: user?.temperament || 'Profile', color: '#6b3991', bg: 'rgba(107,57,145,0.08)' },
        { label: `${user?.personalityVibe || 'Soft'} mode`, color: '#1d6a82', bg: 'rgba(29,106,130,0.08)' },
      ].map(item => (
        <span key={item.label} style={{
          fontSize: 11, fontWeight: 600, color: item.color,
          background: item.bg, padding: '3px 10px', borderRadius: 9999,
...
function Bubble({ msg, userInitial, theme }: { msg: Message; userInitial: string; theme: any }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, marginBottom: 14 }}>
      <div style={{
        ...
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize: 11, fontWeight: 700, color: '#6b3991',
      }}>N</div>
      <div style={{
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', padding: '10px 0 6px' }}>
          <div style={{ width: 36, height: 4, borderRadius: 9999, background: 'rgba(107,57,145,0.15)' }} />
        </div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: '#2f2940', margin: 0, fontFamily: '"DM Serif Display", serif' }}>
              Nudge
            </p>
            <p style={{ fontSize: 11.5, color: '#43e8d8', margin: 0, fontWeight: 600 }}>
        ...
        >
          {isEmpty && (
            <div style={{ textAlign: 'center', padding: '32px 20px 20px' }}>
              ...
              <p style={{ fontSize: 15, fontWeight: 600, color: '#2f2940', marginBottom: 6, fontFamily: '"DM Serif Display", serif' }}>
                Ask Nudge anything
              </p>
              <p style={{ fontSize: 13, color: '#9b8ea5', lineHeight: 1.6, maxWidth: 280, margin: '0 auto' }}>
                Tasks, habits, how you're feeling — Nudge knows your context and responds accordingly.
              </p>
            </div>
          )}

          {messages.map(msg => (
            <Bubble key={msg.id} msg={msg} userInitial={userInitial} theme={theme} />
          ))}
        ...
        {!loading && (isEmpty || messages[messages.length - 1]?.role === 'assistant') && (
          <div style={{
          ...
          >
            {QUICK_REPLIES.map(qr => (
              <button
                ...
                style={{
                  fontSize: 12, fontWeight: 600, color: '#6b3991',
                  background: 'rgba(107,57,145,0.07)',
        ...
        <div style={{
          ...
          <textarea
            ...
            style={{
              ...
              color: '#2f2940', outline: 'none', resize: 'none',
              lineHeight: 1.5, maxHeight: 100, overflowY: 'auto',
              transition: 'border-color 0.18s ease',
            }}
            onFocus={e => (e.target.style.borderColor = '#43e8d8')}
            onBlur={e => (e.target.style.borderColor = 'rgba(107,57,145,0.12)')}
          />
          <button
            onClick={() => sendMessage()}
            ...
            style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 17, color: input.trim() && !loading ? '#fff' : '#9b8ea5',
  ...
  return (
    <button
      ...
      onMouseEnter={e => {
        ...
        ;(e.currentTarget as HTMLElement).style.boxShadow = '0 6px 28px rgba(107,57,145,0.4)'
      }}
      onMouseLeave={e => {
        (e.currentTarget as HTMLElement).style.transform = 'none'
        ;(e.currentTarget as HTMLElement).style.boxShadow = '0 4px 20px rgba(107,57,145,0.35)'
      }}
    >
      {/* Nudge N mark */}
      <span style={{
      ...
      {unreadCount > 0 && (
        <span style={{
          ...
          background: '#43e8d8',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 10, fontWeight: 800, color: '#0d3d38',
          flexShrink: 0,
        }}>
