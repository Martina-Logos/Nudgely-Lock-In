  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        >
          <i
            className={`ti ${collapsed ? 'ti-layout-sidebar-right' : 'ti-layout-sidebar'}`}
            style={{ fontSize: 15 }}
            aria-hidden="true"
          />
        </button>
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: '8px 8px' }}>
        {!collapsed && (
          <p style={{ fontSize: 10, fontWeight: 700, color: labelColor, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '6px 4px 4px' }}>
            Main
          </p>
        )}
        ...
        {!collapsed && (
          <p style={{ fontSize: 10, fontWeight: 700, color: labelColor, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '12px 4px 4px' }}>
            Focus
          </p>
        )}
        {collapsed && <div style={{ width: '100%', height: 1, backgroundColor: borderColor, margin: '6px 0' }} />}
        {NAV_FOCUS.map(item => <NavItem key={item.path} item={item} />)}
      ...
      {/* User section */}
      <div style={{ borderTop: `1px solid ${borderColor}`, padding: '10px 8px' }}>
        <button
          ...
          {!collapsed && (
            <div style={{ textAlign: 'left', overflow: 'hidden', flex: 1 }}>
              ...
              <p style={{ fontSize: 10, color: labelColor }}>
                {user?.personalityVibe === 'Bold' ? 'Bold theme' : 'Soft theme'}
              </p>
            </div>
          )}

          {!collapsed && (
            <i className="ti ti-settings" style={{ fontSize: 14, color: labelColor, flexShrink: 0 }} aria-hidden="true" />
          )}
        </button>
      </div>
    </aside>
        ...
        return (
          <button key={item.path} onClick={() => navigate(item.path)}
            style={{
              ...
              padding: '4px 12px', border: 'none', backgroundColor: 'transparent',
              cursor: 'pointer', borderRadius: 12,
            }}>
            <i className={`ti ${item.icon}`} style={{ fontSize: 22, color }} aria-hidden="true" />
            <span style={{ fontSize: 10, fontWeight: 600, color }}>{item.label}</span>
          </button>
        )
      })}
    </nav>
...
function TopBar({ theme, Aurora }: { theme: any; Aurora: boolean }) {
  ...
  return (
    <header style={{
      ...
      <div>
        <span style={{ fontSize: 17, fontWeight: 700, color: textPri, fontFamily: '"DM Serif Display", serif' }}>
          Nudgely
        </span>
        <span style={{ fontSize: 12, fontWeight: 600, color: theme.accent, marginLeft: 4 }}>Lock-In</span>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        <span style={{ fontSize: 12, color: textSec }}>{today}</span>
        <button
          onClick={() => navigate('/profile')}
          style={{
            width: 34, height: 34,
